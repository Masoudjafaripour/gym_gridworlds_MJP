...
def every_visit_mc_prediction(env, policy, gamma, num_episodes):
    """
    Every-visit Monte Carlo prediction algorithm for estimating V ≈ v_π.
    ...
    for episode in range(num_episodes):
        ...
        for t in range(len(episode_data) - 1, -1, -1):
            ...
            # Append G to returns for state_t on **every visit**
...
# Run Every-Visit MC Prediction
V = every_visit_mc_prediction(env, random_policy, gamma, num_episodes)
...
def plot_value_function_heatmap(V, grid_shape=(3, 3)):
    ...
    plt.title('Every-Visit MC Value Function Heatmap')
