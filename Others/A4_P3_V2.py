...
def target_eps_greedy_probs(Q):
    ...
    for s in range(n_states):
        best_action = np.argmax(Q[s])
        probs[s, best_action] += (1 - eps)  # Increase the probability of the best action
    return probs

# Monte Carlo Control with Off-policy Weighted Importance Sampling
def off_policy_mc_control(env, Q, gamma, eps_decay, max_steps, episodes_per_iteration):
    ...
    if len(bellman_errors) > max_steps:
        bellman_errors = bellman_errors[:max_steps]
    else:
