...
def bellman_v(policy, gamma, init_value, max_iterations=10000, tol=1e-7): 
    V = np.full(n_states + 1, init_value)  # Initialize V with init_value, +1 for terminal state
    bellman_errors = []
    ...
    for _ in range(max_iterations):
        ...
        for s in range(n_states):
            ...
            # Bellman update for non-terminal states
            V[s] = np.sum(P[s, a, :] * (R[s, a] + gamma * V_prev))
        
        error = np.max(np.abs(V[:n_states] - V_prev[:n_states]))  # Only care about non-terminal states
        bellman_errors.append(error)
        
        if error < tol:  # Convergence condition
            break
    
    return V[:n_states], bellman_errors  # Return V without the terminal state
...
def bellman_q(gamma, init_value, max_iterations=1000, tol=1e-7):
    ...
    for _ in range(max_iterations):
        ...
        if np.max(np.abs(V[:n_states] - V_prev[:n_states])) < tol:  # Convergence condition for non-terminal states
            break
